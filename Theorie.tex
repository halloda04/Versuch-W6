...
ist, dass es bei 4°C eine Dichteanomalie gibt. Das bedeutet, dass dort 
die Dichte von Wasser am höchsten ist und bei niedrigerer und höherer 
...
\caption{Das p,T-Phasendiagramm von von einem Stoff ohne Dichteanomalie 
und von Wasser mit Dichteanomalie\cite{Phasendiagrammwasser}}
\end{figure}
...
\begin{equation}
    \frac{\Delta p_D}{\Delta T} = \frac{r\left(T\right)}{\left(V_D - V_{Fl}\right)\cdot \left(T + \Delta T\right)}
\label{Carnot-Umgestellt}
\end{equation}

Wenn nun angenommen wird, dass $\Delta T$ gegen 0 geht, kann Gleichung 
...
Um nun den Dampfdruck in Abhängigkeit der Temperatur zu erhalten, wird 
angenommen, dass $V_D \gg V_{Fl}$ und dass $r\left(T\right)$ im betrachteten Temperaturbereich konstant ist,
also $r\left(T\right) = r$. Diese Annahmen zusammen mit Gleichung \ref{Clausius-Clapeyron-Gleichung} ergeben:
\begin{equation}
...
zu erhalten, wird angenommen, dass das Dampfvolumen dem eines idealen 
Gases entspricht. Zusätzlich wird noch mithilfe von Trennung der Variablen 
integriert. Dadurch erhält man für $p_D$:
\begin{equation}
    p_D = p_D^0 \cdot \exp\left[-\frac{r}{R}\cdot \left(\frac{1}{T} - \frac{1}{T_0}\right)\right]
    \label{eq:pd}
\end{equation}

Hierbei ist $p_D$ der neue Dampfdruck, $p_D^0$ der Dampfdruck des 
Anfangszustands, $r$ die Verdampfungswärme, $R$ die allgemeine Gaskonstante, 
$T$ die neue Temperatur und $T_0$ die Anfangstemperatur.
